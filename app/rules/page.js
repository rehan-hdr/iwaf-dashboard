...
export default function RulesPage() {
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold text-gray-800 mb-2">Rule Management</h1>
      <p className="text-gray-500 mb-6">Configure ModSecurity CRS rules and custom rules</p>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
        {categoryStats.map((stat) => (
          <div
            key={stat.category}
            className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 cursor-pointer hover:border-[#FF7A50] transition-colors"
          ...
          >
            ...
            <div className="text-xl font-bold text-gray-800 mt-1">{stat.triggers.toLocaleString()}</div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 text-left">
                ...
                <th className="px-6 py-3 text-xs font-medium text-gray-500 uppercase">Rule ID</th>
                ...
                <th className="px-6 py-3 text-xs font-medium text-gray-500 uppercase">Triggers</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {filtered.map((rule) => (
                <tr key={rule.id} className={`hover:bg-gray-50 ${!rule.enabled ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-6 py-4 font-mono text-sm text-gray-800">{rule.id}</td>
                  <td className="px-6 py-4 text-sm text-gray-700 max-w-[300px]">{rule.name}</td>
                  <td className="px-6 py-4">
